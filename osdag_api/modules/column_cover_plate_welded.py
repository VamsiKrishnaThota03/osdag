...
from design_type.connection.column_cover_plate import ColumnCoverPlate  # Reusing the same base class
import sys
import os
import typing
...
def validate_input(input_values: Dict[str, Any]) -> None:
    ...
    # Validate Weld.Size
    weld_size = input_values["Weld.Size"]
    if (not isinstance(weld_size, str)  # Check if Weld.Size is a string.
            or not int_able(weld_size)):  # Check if Weld.Size can be converted to int.
        # If any of these conditions fail, raise error.
        raise InvalidInputTypeError(
            "Weld.Size", "str where str can be converted to int.")
...
def create_from_input(input_values: Dict[str, Any]) -> ColumnCoverPlate:
    ...
    try:
        # Add connection type as welded
        input_values["Connection"] = "Welded"
...
def generate_output(input_values: Dict[str, Any]) -> Dict[str, Any]:
    ...
    raw_output_weld_capacity = module.weld_capacity_details(True) if hasattr(module, 'weld_capacity_details') else []
    logs = module.logs
